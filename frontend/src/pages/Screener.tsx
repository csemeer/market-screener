...
// Screener Stock Modal Component
function ScreenerStockModal({
  stock,
  onClose,
  onAddToWatchlist,
}: {
  ...
  const getCurrency = () => {
    return ['NYSE', 'NASDAQ'].includes(stock.exchange) ? 'USD' : 'INR';
  };
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 sm:p-6 z-10 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-1 truncate">{stock.exchange}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 sm:mt-4">
            ...
            <span className="px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-bold bg-primary-100 text-primary-800">
              Score: {(stock.combinedScore || stock.score)}/100
            </span>
            {stock.recommendation && (
              <span className={`px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-bold ${
                stock.recommendation === 'STRONG_BUY' ? 'bg-green-600 text-white' :
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            <div className="bg-blue-50 rounded-lg p-3 sm:p-4 border border-blue-200">
              <p className="text-xs sm:text-sm text-blue-700 font-medium mb-1">Current Price</p>
              <p className="text-xl sm:text-2xl font-bold text-blue-900">{getCurrencySymbol()}{stock.price.toFixed(2)}</p>
            </div>
            <div className="bg-purple-50 rounded-lg p-3 sm:p-4 border border-purple-200">
              <p className="text-xs sm:text-sm text-purple-700 font-medium mb-1">Confluence Score</p>
              <p className="text-xl sm:text-2xl font-bold text-purple-900">
                {stock.confluenceScore ? `${stock.confluenceScore.toFixed(0)}%` : 'N/A'}
              </p>
            </div>
            <div className="bg-green-50 rounded-lg p-3 sm:p-4 border border-green-200">
              <p className="text-xs sm:text-sm text-green-700 font-medium mb-1">Volume</p>
            ...
            <div>
              ...
              <StockEvidenceChart
                data={{
                  ...
                  historicalPrices: stock.historicalData,
                  indicators: stock.indicators,
                  levels: stock.riskReward ? {
                    entry: stock.riskReward.entryPrice,
                    stopLoss: stock.riskReward.stopLoss,
                    target: stock.riskReward.target,
                  } : undefined,
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {stock.indicators.rsi && (
                  <div className="bg-gray-50 rounded p-3">
                    ...
                    <p className="text-lg font-bold text-gray-900">{stock.indicators.rsi.toFixed(1)}</p>
                  </div>
                )}
                {stock.indicators.adx && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">ADX</p>
                    <p className="text-lg font-bold text-gray-900">{stock.indicators.adx.toFixed(1)}</p>
                  </div>
                )}
                {stock.indicators.macd && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">MACD</p>
                    <p className={`text-lg font-bold ${stock.indicators.macd.histogram > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {stock.indicators.macd.histogram > 0 ? 'Bullish' : 'Bearish'}
                    </p>
                  </div>
                )}
                {stock.indicators.volumeProfile && (
                  <div className="bg-gray-50 rounded p-3">
                    ...
                    <p className="text-lg font-bold text-gray-900">{stock.indicators.volumeProfile.volumeRatio.toFixed(2)}x</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {stock.fundamentals && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Fundamental Analysis</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {stock.fundamentals.peRatio && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">P/E Ratio</p>
                    <p className="text-lg font-bold text-gray-900">{stock.fundamentals.peRatio.toFixed(2)}</p>
                  </div>
                )}
                {stock.fundamentals.pbRatio && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">P/B Ratio</p>
                    <p className="text-lg font-bold text-gray-900">{stock.fundamentals.pbRatio.toFixed(2)}</p>
                  </div>
                )}
                {stock.fundamentals.roe && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">ROE</p>
                    <p className="text-lg font-bold text-gray-900">{stock.fundamentals.roe.toFixed(1)}%</p>
                  </div>
                )}
                {stock.fundamentals.debtToEquity !== undefined && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">Debt/Equity</p>
                    <p className="text-lg font-bold text-gray-900">{stock.fundamentals.debtToEquity.toFixed(2)}</p>
                  </div>
                )}
                {stock.fundamentals.revenueGrowth && (
                  <div className="bg-gray-50 rounded p-3">
                    <p className="text-xs text-gray-600 mb-1">Revenue Growth</p>
                    <p className={`text-lg font-bold ${stock.fundamentals.revenueGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {stock.fundamentals.revenueGrowth.toFixed(1)}%
                    </p>
                  </div>
                )}
                {stock.fundamentals.epsGrowth && (
                  <div className="bg-gray-50 rounded p-3">
                    ...
                    <p className={`text-lg font-bold ${stock.fundamentals.epsGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {stock.fundamentals.epsGrowth.toFixed(1)}%
                    </p>
                  </div>
                )}
          ...
          {stock.signals && stock.signals.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {stock.signals.map((signal: string, index: number) => (
          ...
          {stock.patterns && stock.patterns.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Candlestick Patterns</h3>
              <div className="flex flex-wrap gap-2">
                {stock.patterns.map((pattern: string, index: number) => (
